...
public class ClassDropdownManager : MonoBehaviour
{
    ...
    // Reference to the prefab of the dropdown item
    public GameObject dropdownItemPrefab;

    // Height of the dropdown items
    public float dropdownItemHeight = 40f;
    ...
    void PopulateClassNameDropdown()
    {
        ...
        // Set the height and auto size for the dropdown items
        StartCoroutine(SetDropdownItemProperties());
    }

    IEnumerator SetDropdownItemProperties()
    {
        // Wait for a short time to ensure that the dropdown items are generated
        yield return null;

        // Get the content object within the dropdown's template
        Transform content = classNameDropdown.template.Find("Viewport/Content");

        if (content == null)
        {
            Debug.LogError("Content object not found in the dropdown template.");
            yield break;
        }

        // Loop through the children of the content object and adjust their properties
        foreach (Transform child in content)
        {
            // Get the RectTransform of the dropdown item
            RectTransform itemTransform = child.GetComponent<RectTransform>();

            if (itemTransform == null)
            {
                Debug.LogError("RectTransform not found in dropdown item.");
                continue;
            }

            // Adjust the height of the dropdown item
            itemTransform.sizeDelta = new Vector2(itemTransform.sizeDelta.x, 40f);

            // Get the TextMeshProUGUI component of the dropdown item
            TextMeshProUGUI text = child.GetComponentInChildren<TextMeshProUGUI>();

            if (text != null)
            {
                // Set auto size for the text
    ...
    void OnClassNameDropdownValueChanged(TMP_Dropdown dropdown)
    {
        ...
        StartCoroutine(SetDropdownItemProperty());

    }

    IEnumerator SetDropdownItemProperty()
    {
        // Wait for a short time to ensure that the dropdown items are generated
        yield return null;

        // Get the content object within the dropdown's template
        Transform content = classNumberDropdown.template.Find("Viewport/Content");
